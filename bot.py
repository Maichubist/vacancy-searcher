...
async def job_proccessor(message: types.Message):
    profession = (message.text).replace('Професія: ', '') if (message.text).startswith(' ') else (message.text).replace(
        'Професія:', '')
    ...
    async def city_proccessor(callback_query: types.CallbackQuery):
        ...
        result_w = WorkUaParser(vacancy=users[f'{callback_query.from_user.id}']['profession'],
                              city=users[f'{callback_query.from_user.id}']['city']).get_result()
        result_d = DjiniParser(vacancy=users[f'{callback_query.from_user.id}']['profession'],
                              city=users[f'{callback_query.from_user.id}']['city']).get_result()
        await bot.send_message(callback_query.from_user.id, f'{result_w}, {result_d}, {users}')
        del users[f'{callback_query.from_user.id}']
